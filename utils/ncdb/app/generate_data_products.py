...
def generate_interactive_surface_plot(
    server, 
    dataset, 
    cycle, 
    variable_path, 
    ...
    if not variable_path:
        return
    # product_name = safe_name(variable_path)
    product_name = f"int_{safe_name(variable_path)}"
    ...
    # Path for interactive HTML (change the extensionfrom png)
    html_path = plot_path.with_suffix(".html")

    if not regenerate and os.path.exists(html_path):
        return
    ...
    logger.info(f"Generating interactive plot {product_name} / {obs_space_name} --> {html_path}")

    plotter = PlotGenerator("")
    plotter.generate_interactive_surface_map(html_path, plot_data)
        ...
        for cycle in dataset.dataset_cycles:
            for file in cycle.files:
                ...
                if (variable_name != "waterTemperature") and (variable_name != "salinity"):
                    ...
                    generate_interactive_surface_plot(
                        server,
                        dataset, 
                        cycle, 
                        variable_path, 
