...
class FileScanner:
    ...
    def select_and_parse(
        ...
        """
        Returns list of (File, obs_space_name)
        """
        pattern = parser.get_search_pattern(prefix, hour)
        selected, _ = self.filter_files(files, pattern)
        # print("selected FILES:", len(selected))

        results = []
        for f in selected:
            name = parser.parse(f.path, prefix)
            if name:
                results.append((f, name))
            # else:
                # print("PARSE FAILED:", f.path)
...
class NcObsSpaceNameParser(ObsSpaceNameParser):
    ...
    def old_parse(self, path: str, prefix: Optional[str] = None) -> Optional[str]:
        filename = os.path.basename(path)
        print(f"parse:   filename = {filename}")
        if not filename.endswith(self.EXTENSION):
            return None

        name = filename[:-len(self.EXTENSION)]
        print(f"parse:   name = {name}")
    ...
    def parse(self, path: str, prefix: Optional[str] = None) -> Optional[str]:
        """
        For datasets where filename = obs_space_name.nc
        and dataset identity comes from directory structure.
        """
        filename = os.path.basename(path)
