    ...
    def run_checks(self):
        ...
        for f in files:
            ...
            if file_errors:
                ...
                # Severity Logic
                if any(k in msg for k in ['Zero', 'Corrupt', 'Structure', 'Time Mismatch', 'Overflow', 'Underflow']):
                    status = 'FAIL'    # Critical Errors (Red)
                elif "INFO:" in msg:
                    status = 'OK'      # Informational (Green/Blue)
                else:
                    status = 'WARNING' # Volume/Quality Warnings (Yellow/Red depending on preference)
        ...
        logger.info(f"Inspection Complete. Found {issues} anomalies in {len(files)} files.")
