...
class StageOutput(Task):
    ...
    def run(self):
        ...
        - If source is GSI: source directory is COMIN_OBSFORGE_ATMOS_GSI, files have suffix .gsi.nc
        - If source is BUFR: source directory is COMIN_OBSFORGE_ATMOS, files have suffix .nc
        - Destination is COMOUT_OBSFORGE_ATMOS_OBS with suffix .nc
        """
        ...
        for obs in self.task_config.observations:
            ...
            if obs_source == 'GSI':
                src_dir = self.task_config.COMIN_OBSFORGE_ATMOS_GSI
                src_suffix = '.gsi.nc'
            elif obs_source == 'BUFR':
                src_dir = self.task_config.COMIN_OBSFORGE_ATMOS
                src_suffix = '.nc'
            else:
            ...
            dest_file = os.path.join(self.task_config.COMOUT_OBSFORGE_ATMOS_OBS, f"{self.task_config.OPREFIX}{obs_name}.nc")
        ...
        if copy_list:
            FileHandler({'mkdir': [self.task_config.COMOUT_OBSFORGE_ATMOS_OBS], 'copy': copy_list}).sync()
            logger.info(f"Copied {len(copy_list)} observation files to {self.task_config.COMOUT_OBSFORGE_ATMOS_OBS}")

            # Write observation source log file
            log_file_path = os.path.join(self.task_config.COMOUT_OBSFORGE_ATMOS_OBS,
        ...
        bufr_status_log_src = os.path.join(self.task_config.COMIN_OBSFORGE_ATMOS,
                                           "obsforge_atmos_bufr_status.log")
        bufr_status_log_dest = os.path.join(self.task_config.COMOUT_OBSFORGE_ATMOS_OBS,
                                            "obsforge_atmos_bufr_status.log")
        ...
        if bias_correction_config.get('source') == 'GSI':
            bc_src_file = os.path.join(self.task_config.COMIN_OBSFORGE_ATMOS_GSI,
